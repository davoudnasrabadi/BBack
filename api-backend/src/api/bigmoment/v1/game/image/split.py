...
class PieceOutLine():
    ...
    def genRightFemaleArc(self, istop):
        ...
        points = [top, (top[0] + dw/3 + 8, top[1] + dh/3),
                  (top[0] + 2*dw/3 + 8, top[1] + dh * 2/3), bottom]
    ...
    def genBottomFemaleArc(self, isLeft):
        ...
        points = [right, (right[0] - dw/3, right[1] + dh/3 + 8),
                  (right[0] - 2*dw/3, right[1] + dh * 2/3 + 8), left]
...
def createPuzzlePieces(name, row, col, outPrefix):
    ...
    outLine = PieceOutLine(
        im.size[0]/col, im.size[1]/row, arcRatio, connectRatio)
    ...
    for i in range(0, row):
        for j in range(0, col):
            ...
            for p in curvPoints:
                outLinePoints.append(
                    (p[0] + j * w + 0.5 * w, p[1] + i * h + 0.5 * h))
    ...
    outLinedraw.rectangle([(0, 0), (im.size[0], im.size[1])],
    ...
    for p in outLinePoints:
        ...
        outLinedraw.ellipse((px - r, py - r, px + r, py + r),
                            fill=lineColor, outline=lineColor)
...
def main():
    ...
    createPuzzlePieces(sys.argv[2], int(
        sys.argv[3]), int(sys.argv[4]), outPrefix)
